...
class OperatingSystem:
    ...
    def get_info(self, driver):
        ...
        # next_news = news.find_element_by_id("feed_rows_next") # Новые новости
        # unshown_news = news.find_element_by_class_name("feed_row_unshown") # Следующие новости
        ...
        for item in items:
            ...
            try:
                ...
                try:
                    ...
                    text = "<-- There are not text. -->"
                    links = "<-- There are not links. -->"
                ...
                try:
                    ...
                    img_links = "<-- There are not images. -->"
            except NoSuchElementException:
                continue
    ...
    def run(self):
        ...
        self.save_data_to_json(result, "file2.json", post_text=False, post_links=False)
